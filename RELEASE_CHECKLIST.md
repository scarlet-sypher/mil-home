...
- Download `rms-home-setup.exe` from the artifacts of a green `build-installer` run
  (GitHub Actions -> that workflow run -> Artifacts -> `rms-home-setup`).
...
- **VM B ("pre-existing software")**: Windows 10/11 with its own Node.js already
  ...
  RMS-HOME. Before installing RMS-HOME on this VM, note that service's name (whatever
...
1. Copy `rms-home-setup.exe` to VM A and double-click it (no terminal, no flags).
...
3. Once it finishes, with **zero manual terminal use**, confirm:
   ...
   - Services (`services.msc`) shows **`rms-home-postgresql`**, status Running.
   - `C:\Program Files\rms-home\` exists and contains `.env`, `launcher\`, and the
     built app files.

**Pass:** wizard completes silently, Node present, `rms-home-postgresql` running, all
without you typing a single install/config command yourself.

## 2. Pre-existing-software pass (VM B)

1. Run `rms-home-setup.exe` on VM B, same as step 1.
...
4. Confirm RMS-HOME's own service, **`rms-home-postgresql`**, is also running,
   registered as its own distinct service, and — since 5432 was already taken by the
   pre-existing instance and RMS-HOME's own default (5433) may or may not also be free
   on this VM — check `C:\Program Files\rms-home\.env`'s `DATABASE_URL` for whichever
...
pre-existing Postgres instance or its data, and RMS-HOME's own instance comes up
cleanly alongside both on its own resolved port.

## 3. First-run flow

1. On either VM, double-click the **RMS-HOME** desktop shortcut.
2. Confirm the default browser actually opens (this is the one visual step CI cannot
   cover — CI has no browser UI session to observe) to the RMS-HOME login page, within
...
1. Log in with the bootstrap admin account: `admin@rmshome.local` /
   `Bharat#Veer_91`.
...
1. With RMS-HOME open in the browser (as the now-reconfigured admin), close the
...
3. Confirm **`rms-home-postgresql`** in `services.msc` is still Running the entire
...
1. With RMS-HOME already open and running, double-click the desktop shortcut again.
...
1. On VM A: run the uninstaller (Windows Settings -> Apps, or `unins*.exe` directly in
   the install folder). Confirm **`rms-home-postgresql`** is gone from Services,
   `C:\ProgramData\RMS-HOME` no longer exists, and **`C:\Program Files\rms-home` itself
...
2. Open Control Panel -> **Programs and Features** (or Settings -> Apps). Confirm
   ...
   **"Node.js" entry is expected to still be there** if this VM's Node was installed by
   RMS-HOME itself — that's the known, deliberate gap from item 3 below, not a bug.
3. On **VM B** — this is the one that actually matters most for this step — run the
   uninstaller there too, then confirm:
   - `rms-home-postgresql`, its data directory, the install directory, and its
...
4. **If this VM's Node was installed by RMS-HOME itself** (not pre-existing), be aware:
